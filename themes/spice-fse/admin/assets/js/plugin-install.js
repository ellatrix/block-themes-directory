jQuery(document).ready(function($) {
    'use strict';
    var this_obj = spice_fse_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'spice_fse_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'spice_fse_hide_customizer_notice';
            sdata.spice_fse_plugin_slug = slug;
        }else{
            sdata.action = 'spice_fse_hide_customizer_companion_notice';
        }
