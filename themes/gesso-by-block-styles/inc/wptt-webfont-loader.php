...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return bool|array<string> Returns an array of remote URLs and their local counterparts.
		 */
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				if ( is_array( $files ) ) {
					foreach ( $files as $url ) {
						...
						// Get the filename.
						$filename = wp_parse_url( $url, PHP_URL_PATH );
						...
						if ( file_exists( $font_path ) ) {
							...
							if ( is_array( $stored ) && isset( $stored[ $url ] ) ) {
								continue;
							}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array<mixed> Returns an array of font-families and the font-files used.
		 */
