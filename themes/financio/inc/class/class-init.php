...
use Financio\Block_Patterns;
use Financio\Block_Styles;
use Financio\Upgrader;
...
class Init {
	...
	public function notice_install_plugin() {
		...
		$button_text = __( 'Check it Now!', 'financio' );
		$button_link = wp_nonce_url( self_admin_url( 'themes.php?page=financio-dashboard' ), 'install-plugin_gutenverse' );
		?>
		<style>
			.install-gutenverse-plugin-notice {
				...
				position: relative;
				overflow: hidden;
				background-image: url(<?php echo esc_url( FINANCIO_URI . '/assets/images/mockup-2x.png' ); ?>);
		...
		jQuery( function( $ ) {
			$( 'div.notice.install-gutenverse-plugin-notice' ).on( 'click', 'button.notice-dismiss', function( event ) {
				...
				$.post( ajaxurl, {
					action: 'financio_set_admin_notice_viewed'
				} );
			} );
		} );
		</script>
		<div class="notice is-dismissible install-gutenverse-plugin-notice">
			<div class="gutenverse-notice-inner">
				<div class="gutenverse-notice-content">
					<h3><?php esc_html_e( 'Thank you for installing Financio!', 'financio' ); ?></h3>
					<p><?php esc_html_e( 'Financio theme work best with Gutenverse plugin. By installing Gutenverse plugin you may access Financio templates built with Gutenverse and get access to more than 40 free blocks.', 'financio' ); ?></p>
					<div class="gutenverse-bottom">
						...
						<a target="__blank" href="https://gutenverse.com/">
							<?php esc_html_e( 'More Info', 'financio' ); ?>
	...
	public function admin_menu() {
		add_theme_page(
			...
			'manage_options',
			'financio-dashboard',
			array( $this, 'load_financio_dashboard' ),
			1
		);
	}

	/**
	 * Financio Template page
	 */
	public function load_financio_dashboard() {
		?>
			<?php if ( defined( 'GUTENVERSE_VERSION' ) && version_compare( GUTENVERSE_VERSION, '1.1.1', '<=' ) ) { ?>
			<div class="notice is-dismissible">
				<span>
				<?php echo esc_html_e( 'Please install newer version of Gutenverse plugin! (v1.1.2 and above)', 'financio' ); ?>
				</span>
			</div>
			<?php } ?>
	...
	public function theme_setup() {
		load_theme_textdomain( 'financio', FINANCIO_DIR . '/languages' );
		...
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary', 'financio' ),
			)
		);
	...
	public function enqueue_scripts() {
		...
		wp_add_inline_style( 'financio-style', $this->load_font_styles() );

		// enqueue additional core css.
		wp_enqueue_style( 'financio-core-add', FINANCIO_URI . '/assets/css/core-add.css', array(), FINANCIO_VERSION );

		// enqueue core animation.
		wp_enqueue_script( 'financio-animate', FINANCIO_URI . '/assets/js/index.js', array(), FINANCIO_VERSION, true );
		wp_enqueue_style( 'financio-animate', FINANCIO_URI . '/assets/css/animation.css', array(), FINANCIO_VERSION );
	...
	public function dashboard_scripts() {
		...
		wp_enqueue_style(
			'financio-notice',
			FINANCIO_URI . '/assets/css/notice.css',
			array(),
			FINANCIO_VERSION
		);

		if ( $screen->id === 'appearance_page_financio-dashboard' ) {
			// enqueue css.
			wp_enqueue_style(
				'financio-dashboard',
				FINANCIO_URI . '/assets/css/dashboard.css',
				array(),
				FINANCIO_VERSION
			);
			...
			wp_enqueue_script(
				'financio-dashboard',
				FINANCIO_URI . '/assets/js/dashboard.js',
				array( 'wp-api-fetch' ),
				FINANCIO_VERSION,
				true
			);
	...
	public function theme_config() {
		return array(
			...
			'title'        => esc_html__( 'Get Advanced Templates for Free!', 'financio' ),
			'description'  => esc_html__( 'By just installing and activate Gutenverse plugin you will be able to use this theme\'s advanced templates. With Gutenverse plugin installed, you gain access to both advance version of templates and block patterns for free which all built using Gutenverse blocks.', 'financio' ),
			'title2'       => esc_html__( 'Comparison using Gutenverse vs WordPress Core', 'financio' ),
			'title3'       => esc_html__( 'Benefits for Installing Financio\'s Gutenverse Version', 'financio' ),
			'description3' => esc_html__( 'You can customize your website instantly with powerful and lightweight add-ons plugin for Gutenberg/FSE.', 'financio' ),
			'note'         => esc_html__( 'Note: Clicking the button will both install and activate Gutenverse plugin and templates for this theme. Please first backup your current templates if you have any changes to it.', 'financio' ),
			'note2'        => esc_html__( 'Note 2: (Gutenverse version 1.6.0 & above required!)', 'financio' ),
			'demo'         => esc_html__( 'View Live Demo →', 'financio' ),
			...
			'pages'        => array(
				'home'     => FINANCIO_URI . '/assets/img/page-home.webp',
				'about'    => FINANCIO_URI . '/assets/img/page-about.webp',
				'services' => FINANCIO_URI . '/assets/img/page-services.webp',
				'contact'  => FINANCIO_URI . '/assets/img/page-contact.webp',
				'faq'      => FINANCIO_URI . '/assets/img/page-faq.webp',
			),
			'table'        => array(
				'titles'   => array(
					null,
					esc_html__( 'Gutenverse (FREE)', 'financio' ),
					esc_html__( 'WordPress Core', 'financio' ),
				),
				'features' => array(
					esc_html__( 'Advanced Templates', 'financio' ),
					esc_html__( 'Responsive Styling', 'financio' ),
					esc_html__( 'Variety of Fonts', 'financio' ),
					esc_html__( 'Icon Library', 'financio' ),
					esc_html__( 'Animation Effects', 'financio' ),
					esc_html__( 'Form Builder', 'financio' ),
				),
			),
			'benefits'     => array(
				'title'    => esc_html__( 'Features', 'financio' ),
				'features' => array(
					esc_html__( 'Modern and clean design', 'financio' ),
					esc_html__( '5+ Ready to use templates', 'financio' ),
					esc_html__( '15+ template parts', 'financio' ),
					esc_html__( 'Fully responsive layout', 'financio' ),
					esc_html__( 'Fully customizable', 'financio' ),
				),
			),
		);
	}
