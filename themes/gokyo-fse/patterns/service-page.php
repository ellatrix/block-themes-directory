<?php
 /**
  * Title: Service Page
  * Slug: gokyo-fse/service-page
  * Categories: gokyo-fse
  */
?>
<!-- wp:group {"align":"full","style":{"spacing":{"padding":{"top":"0px","right":"0px","bottom":"0px","left":"0px"},"blockGap":"0px"}}} -->
<div class="wp-block-group alignfull" style="padding-top:0px;padding-right:0px;padding-bottom:0px;padding-left:0px">
<!-- wp:pattern {"slug":"gokyo-fse/cover-with-post-title"} /-->
<!-- wp:pattern {"slug":"gokyo-fse/services"} /-->
<!-- wp:pattern {"slug":"gokyo-fse/promotinal-headline"} /-->
<!-- wp:pattern {"slug":"gokyo-fse/about-info"} /-->

</div>
<!-- /wp:group -->
